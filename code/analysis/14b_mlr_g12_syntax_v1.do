...
import delim using "g12_mlr_complete_dta.csv", clear
...
foreach num in 1 3 4 {
	...
	foreach var of varlist llc_sex age_cat_numeric llc_ethnic3 functional_limitation_cat{
		...
		etable, cstat(_r_b) cstat(_r_se) cstat(_r_ci) cstat(_r_p) showstars showstarsnote title(Cluster `num'.  G12) export(cluster`num'_`var'_g12_complete.xlsx, replace)
	}
